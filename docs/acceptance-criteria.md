...
- [x] I can create a match in each currently-supported format: 1v1, FFA, Pentagram — confirmed with the team: `2v2`/`3v3` being inactive is intentional, not a bug. Current supported roster is 1v1, FFA, and Pentagram; 2-Headed Giant (4 players, 2 teams of 2) is planned as the next format (upcoming, not yet built) and will be a distinct format from the sidelined `2v2` row, not a rename of it. See REQUIREMENTS.md's Supported Formats section and `ROADMAP.md` Phase 12.
...
- [x] Confirming a match updates my global rating and every collection rating for collections that match belongs to — **fixed, was a partial fail with two distinct bugs at the time of testing:**
...
- [x] On approval, the slot becomes mine and I can confirm it, triggering my rating update — **fixed, was a real bug at the time of testing:** after player1 approved, the slot's `user_id`/`claimed_by` correctly became the new account and it sat "1/2 Pending" as expected. The new user then confirmed via "Update Deck & Confirm" (had to create a deck first — the modal offers no "Unknown Deck"/skip option, it just disables Confirm with "No decks available" until you do). `participant_status` became `confirmed`, but **no `rating_history` row was ever written for them** and the match's `ratings_applied_at` stayed frozen at its original pre-claim timestamp — their displayed delta stayed labeled "Estimated" forever because it was only ever computed live for display, never persisted. This was the clearest reproduction in that pass of "confirming doesn't actually update my rating."
...
1. Match detail page is missing the bracket-level visual for individual participant decks. — **STILL PRESENT, not yet fixed.** Confirmed via screenshot: the Participants list on `/match/[id]` shows name + commander only, no bracket badge, even though the deck manager renders the same bracket badge correctly elsewhere.
2. Updating a deck's bracket on a confirmed match doesn't correctly recalculate ratings. — **FIXED 2026-09-15.** Recalc math itself was already correct (verified the dirty-match "Run Recalc Now" debug action produces exactly the formula-predicted delta/rating). The real bug was upstream — ratings weren't getting applied in the first place — which traced back to the match-confirmation redesign work (see §3/§4 and `docs/superpowers/specs/2026-09-15-match-confirmation-redesign-design.md`). As part of that work, `mark_match_dirty()`'s guard was also fixed: it previously only flagged a match dirty 24h+ after creation (a leftover from the abandoned lock-window design), which silently missed most real-world bracket edits.

---

## Summary of confirmed bugs (ranked by significance)

**Fixed 2026-09-15** (match-confirmation redesign — see `docs/superpowers/specs/2026-09-15-match-confirmation-redesign-design.md`):

1. ~~Match confirmation doesn't gate ratings the way it's documented to.~~ **FIXED.** `logMatch()` previously auto-confirmed every real participant and applied ratings synchronously at creation, with no reachable "pending confirmation" state. Now friendship-gated: the reporter and any already-friend participants confirm+rate immediately, everyone else stays genuinely pending until they confirm or become friends with the reporter. See §3.
2. ~~Confirming a claimed slot never triggers a rating update.~~ **FIXED.** Claim confirmation now goes through the same shared `applyParticipantRating()` helper as every other confirmation path. See §11.

**Still open** (not in scope for the confirmation redesign):
...
**Not a bug** (confirmed as intentional design with the team):

7. ~~2v2 and 3v3 formats are configured inactive.~~ Intentional — current supported roster is 1v1, FFA, and Pentagram. 2v2/3v3 are sidelined (kept in the schema, may return later); 2-Headed Giant is the planned next format, distinct from the sidelined 2v2. See §3, `REQUIREMENTS.md`'s Supported Formats section, and `ROADMAP.md` Phase 12.
